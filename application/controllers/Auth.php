...
class Auth extends CI_Controller
{
    ...
    private function _login()
    {
        ...
        if ($user) {
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Email is no registered!</div>');
            redirect('auth');
        }
    }

    public function registration() //mengarah ke registration.php
    {
        if ($this->session->userdata('email')) {
            redirect('user');
        }
        ...
        if ($this->form_validation->run() == false) {
            $this->load->library('form_validation');
            $data['title'] = 'User Registration'; //mengarah ke auth_header.php
            ...
            $this->session->set_flashdata('message', '<div class="alert alert-success" role="alert">Congratulation! your account has been created. Please activate your account in gmail
          </div>');
            redirect('auth');
        }
    }
    ...
    private function _sendEmail($token, $type)
    {
        $config = [
            ...
            'smtp_user' => 'aryanurarrif@gmail.com',  // Email gmail
            'smtp_pass'   => 'Watugudik27',  // Password gmail
            'smtp_port'   => 465,
            'crlf'    => "\r\n",
            'newline' => "\r\n"
        ];
        ...
        $this->email->from('aryanurarrif@gmail.com', 'AryaNurarif');
    ...
    public function verify()
    {
        ...
        if ($user) { //agar tidak di jailkan oleh user lain
    ...
    public function forgotpassword()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($user) {
                ...
                $this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">Your email is not registered or activated
        </div>');
                redirect('auth/forgotpassword');
            }
        }
    ...
    public function resetPassword()
    {
        ...
        $user = $this->db->get_where('user', ['email' => $email])->row_array();   //buat cek email di database
